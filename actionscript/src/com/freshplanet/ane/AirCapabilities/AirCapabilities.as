...
public class AirCapabilities extends EventDispatcher {
    ...
    public function uniqueID():String {
        ...
        return _extContext.call("uniqueID") as String;
    }

    public function getReferrer():String {
        if (!isSupported)
            return "";
        return _extContext.call("getReferrer") as String;
    }

    public function openApplicationSetting():Boolean {
        if (!isSupported)
            return false;

        return _extContext.call("openApplicationSetting");
    }

    public function openApplication(id:String):Boolean {
        if (!isSupported)
            return false;

        return _extContext.call("openApplication", id);
    }

    public function setStatusBarStyle(light:Boolean):void {
        if (isSupported) {
            _extContext.call("setStatusBarStyle", light);
        }
    }

    public function setStatusBarHidden(hide:Boolean):void {
        if (isSupported) {
            _extContext.call("setStatusBarHidden", hide);
        }
    }

    public function getAvailableDevices():Vector.<CaptureDevice> {
        if (!isSupported)
            return null;

        return _extContext.call("getAvailableDevices") as Vector.<CaptureDevice>;
    }

    public function requestAccessForMediaType(mediaType:String, requestAccess:Boolean):Boolean {
        if (!isSupported)
            return false;
        return _extContext.call("requestAccessForMediaType", mediaType, requestAccess);
    }
	public function getInstallerPackageName():String {
        if (!isSupported)
            return "";

        return _extContext.call("getInstallerPackageName") as String;
    }
